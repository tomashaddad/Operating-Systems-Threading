...
std::vector<std::string> Task2::sortFrom(int charPos) {
    ...
    std::sort(start, end, [](const std::string &a, const std::string &b) {
...
void Task2::writeToFile(int charPos, const std::vector<std::string> &list) {
    std::string dest =
        "src/task2/sorted/sorted_" + std::to_string(charPos) + ".txt";
...
void Task2::reduce2() {
    ...
    for (auto i = MIN_LENGTH; i <= MAX_LENGTH; ++i) {
        std::string source =
            "src/task2/sorted/sorted_" + std::to_string(i) + ".txt";
        std::ifstream in(source);

        if (!in) {
            std::cerr << "Failed to open input stream for file: " << source
                      << ". Aborting ..." << std::endl;
        ...
        std::cout << "Read " << list.size() << " words from " << source
                  << std::endl;
    }
    ...
    while (counter < sum) {
        ...
        auto min =
            std::min_element(std::begin(currentWords), std::end(currentWords),
